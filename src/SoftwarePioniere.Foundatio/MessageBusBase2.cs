...
{
    public abstract class MessageBusBase2<TOptions> : MaintenanceBase, IMessageBus2 where TOptions : SharedMessageBusOptions
    {
        ...
        public MessageBusBase2(TOptions options) : base(options?.LoggerFactory)
        {
        ...
        public async Task SubscribeAsync<T>(Func<T, CancellationToken, Task> handler, Func<T, bool> messagefilter, CancellationToken cancellationToken = default) where T : class
        {
            if (_logger.IsEnabled(LogLevel.Trace))
