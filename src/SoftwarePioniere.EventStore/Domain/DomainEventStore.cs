...
namespace SoftwarePioniere.EventStore.Domain
{
    public class DomainEventStore : IEventStore
    {
        ...
        public DomainEventStore(ILoggerFactory loggerFactory, EventStoreConnectionProvider provider) : this(loggerFactory, provider,
            SoftwarePioniere.Domain.Util.AggregateIdToStreamName) //, Util.EventTypeToEventName)
        {
        }
    ...
    where T : AggregateRoot
        {
            ...
            var eventsToSave = domainEvents.Select(e => MessageExtensions.ToEventData(e, eventHeaders)).ToList();
